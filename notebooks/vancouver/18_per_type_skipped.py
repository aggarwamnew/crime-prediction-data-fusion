"""18_per_type_skipped.py (exploratory, relaxed threshold)

Retries the three property types skipped in script 17 at the standard >=12-crime gate
(they had <50 active DAs). Each runs at the lowest crime threshold giving >=50 DAs, so
results are exploratory and not directly comparable to the >=12 per-type table.

Adapted from 17_per_type_extended.py — Vancouver per-crime-type fusion, extended to the parity layers.
...
Merges the new columns into data/processed/vancouver/per_type_skipped_exploratory.csv.
"""
import sys
import zipfile
...
# ── CIMD deprivation scores (the four dimension scores) ──
cimd_cols = [c for c in cimd.columns if c.strip().endswith("Scores")]
cimd_dep = cimd[[da_col] + cimd_cols].rename(columns={da_col: "DAUID"})
cimd_dep["DAUID"] = cimd_dep["DAUID"].astype(str).str.replace(r"\.0$", "", regex=True)
for c in cimd_cols:
    cimd_dep[c] = pd.to_numeric(cimd_dep[c], errors="coerce")

# ── weather (ECCC YVR, monthly) ──
...
wcol = lambda s: [c for c in w.columns if s.lower() in c.lower()][0]
w["month"] = pd.to_datetime(w[wcol("Date/Time")]).dt.strftime("%Y-%m")
for cc in (wcol("Max Temp"), wcol("Min Temp"), wcol("Total Precip")):
    w[cc] = pd.to_numeric(w[cc], errors="coerce")
wagg = w.groupby("month").agg(tmax_mean=(wcol("Max Temp"), "mean"), tmin_mean=(wcol("Min Temp"), "mean"),
                              prcp_total=(wcol("Total Precip"), "sum")).reset_index()
...
jj = gpd.sjoin(gp, da_g[["DAUID", "geometry"]], how="inner", predicate="within")
...
poi_counts["poi_total"] = poi_counts[poi_cols].sum(axis=1); poi_cols = poi_cols + ["poi_total"]
...
LSPECS = {
    "CIMD": ("static", cimd_dep, cimd_cols),
    "Weather": ("monthly", wagg, wcols),
    "POI": ("static", poi_counts[["DAUID"] + poi_cols], poi_cols),
...
# Exploratory retry of the three types skipped at the standard >=12-crime / >=50-DA gate.
# Each is run at the lowest crime threshold that yields >=50 active DAs (documented as such).
TYPES = ["Break and Enter Residential/Other", "Theft of Bicycle", "Theft of Vehicle"]
MINTHRESH = {"Break and Enter Residential/Other": 8, "Theft of Bicycle": 8, "Theft of Vehicle": 6}
...
for t in TYPES:
    ...
    thr = MINTHRESH[t]
    active = totals[totals >= thr].index
    if len(active) < 50:
    ...
    row = {"type": t, "n_das": len(active), "min_crimes": thr}
...
res = pd.DataFrame(rows_out).set_index("type")
res.to_csv(PROC / "per_type_skipped_exploratory.csv")
pd.set_option("display.width", 250, "display.max_columns", 25)
print("\nFULL PER-TYPE TABLE (all layers):")
print(res.round(4).to_string())
print("\nBest layer per type:")
layer_cols = [c for c in res.columns if c not in ("base_R2", "n_das", "min_crimes")]
for t in res.index:
