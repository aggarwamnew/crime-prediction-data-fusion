"""Data loader for experimental scripts. READ-ONLY access to existing data."""
...
def load_full_fusion_data():
    """Load all 51 features + crime counts. Returns train, test DataFrames and feature lists."""
    db = ThesisDB()
    ...
    for w in [3,6,12]:
        df[f'rolling_mean_{w}'] = df.groupby('lsoa_code')['crime_count'].transform(lambda x: x.shift(1).rolling(w, min_periods=1).mean())
    ...
    base_feats = ['lag_1','lag_2','lag_3','lag_6','lag_12','rolling_mean_3','rolling_mean_6','rolling_mean_12','month_sin','month_cos','time_idx']
    ...
    # Housing (tertile bands)
    xlsx = PROJECT_ROOT/"data/raw/london/housing/ons_house_prices_lsoa.xlsx"
    hp = pd.read_excel(xlsx, skiprows=4, header=None, names=['la_code','la_name','lsoa_code','lsoa_name','median_house_price'])
    hp = hp[hp['la_code'].astype(str).str.startswith('E09')].copy()
    hp['median_house_price'] = pd.to_numeric(hp['median_house_price'], errors='coerce')
    hp = hp[['lsoa_code','median_house_price']].dropna()
    t = hp['median_house_price'].quantile([1/3,2/3])
    hp['price_low'] = (hp['median_house_price']<t.iloc[0]).astype(int)
    hp['price_mid'] = ((hp['median_house_price']>=t.iloc[0])&(hp['median_house_price']<t.iloc[1])).astype(int)
    hp['price_high'] = (hp['median_house_price']>=t.iloc[1]).astype(int)
    ...
    # Merge all
    for d in [imd, demo, poi, hp[['lsoa_code']+housing_feats], samhi, edu[['lsoa_code']+edu_feats], hh[['lsoa_code']+hh_feats]]:
    ...
    return train, test, base_feats, all_feats
