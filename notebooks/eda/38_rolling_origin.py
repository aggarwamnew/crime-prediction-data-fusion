"""38_rolling_origin.py — Rolling-origin (backtest) robustness of the headline results.

The thesis headline R^2 and Delta R^2 are reported on a single temporal test window
(the last six months). This script re-evaluates the crime-only baseline and the fusion
lifts across several rolling-origin test windows, sliding a six-month test period back
through the series and training only on data strictly preceding each window. If the
baseline R^2 stays in a tight band and every fusion lift stays negligible across all
windows, the conclusions do not depend on the particular split chosen.

Reuses the exact data build and Random Forest configuration of 20_full_fusion.py.
"""
...
# ═══ LOAD ALL DATASETS (identical to 20_full_fusion.py) ═══
...
all_supp = all_context + temporal_feats + all_ss

# ═══ BUILD PANEL (identical to 20_full_fusion.py) ═══
crime = db.query("SELECT lsoa_code, month, COUNT(*) as crime_count FROM crime_clean GROUP BY lsoa_code, month")
active = crime.groupby('lsoa_code')['crime_count'].sum(); active = active[active>=36].index
...
ts = pd.to_datetime(df['month']); df['time_idx'] = (ts.dt.year-ts.dt.year.min())*12+ts.dt.month
...
def fit_r2(train, test, feats):
    valid = [f for f in feats if f in train.columns]
    rf = RandomForestRegressor(**RF); rf.fit(train[valid], train['crime_count'])
    return r2_score(test['crime_count'], rf.predict(test[valid]))

# ═══ ROLLING-ORIGIN WINDOWS ═══
# featured months are indices 12..35; slide a 6-month test window back in 3-month steps,
# training only on featured months strictly before each window.
starts = [30, 27, 24, 21, 18]
rows = []
print("="*90)
print("ROLLING-ORIGIN BACKTEST (6-month test windows, train = all featured months before)")
print("="*90)
print(f"{'Test window':>17s} | {'train mo':>8s} | {'Base R2':>8s} | {'d Weather':>9s} | {'d IMD':>7s} | {'d Full':>7s}")
print("-"*90)
for s in starts:
    test_months = all_months[s:s+6]
    train_months = all_months[12:s]
    tr = df_model[df_model['month'].isin(train_months)]
    te = df_model[df_model['month'].isin(test_months)]
    base = fit_r2(tr, te, base_feats)
    d_weather = fit_r2(tr, te, base_feats+weather_feats) - base
    d_imd = fit_r2(tr, te, base_feats+imd_feats) - base
    d_full = fit_r2(tr, te, base_feats+all_supp) - base
    win = f"{test_months[0]}..{test_months[-1]}"
    rows.append({'window':win,'train_months':len(train_months),'base_r2':base,
                 'd_weather':d_weather,'d_imd':d_imd,'d_full':d_full})
    print(f"{win:>17s} | {len(train_months):>8d} | {base:>8.4f} | {d_weather:>+9.4f} | {d_imd:>+7.4f} | {d_full:>+7.4f}", flush=True)

res = pd.DataFrame(rows)
out = PROJECT_ROOT/"data/processed/london/rolling_origin.csv"
res.to_csv(out, index=False)
print("-"*90)
print(f"Baseline R2: mean {res['base_r2'].mean():.4f}, range [{res['base_r2'].min():.4f}, {res['base_r2'].max():.4f}]")
print(f"Full-fusion d R2: mean {res['d_full'].mean():+.4f}, range [{res['d_full'].min():+.4f}, {res['d_full'].max():+.4f}]")
print(f"Weather d R2: mean {res['d_weather'].mean():+.4f}, range [{res['d_weather'].min():+.4f}, {res['d_weather'].max():+.4f}]")
print(f"Saved -> {out}")
